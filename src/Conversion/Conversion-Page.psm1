    ...
    process {
        ...
        try {
            ...
            foreach ($image in $images) {
                ...
                # Change paths to images in markup file
                try {
                    "Mutation of markup: Rename image references to unique name. Find '$( $image.Name )', Replacement: '$( $newimageName )'" | Write-Verbose
                    # ---------------------------------------------------------------------------
                    # Use -LiteralPath so that characters like '(', ')', '[', ']', '`', "'", '"'
                    # are supported. Or else we will get an error "Cannot find path 'xxx' because
                    # it does not exist"
                    # ---------------------------------------------------------------------------
                    $content = Get-Content -LiteralPath $pageCfg['filePath'] -Raw -ErrorAction Stop -Encoding UTF8
                    ...
                    # it does not exist"
                    # ---------------------------------------------------------------------------
                    Set-ContentNoBom -LiteralPath $pageCfg['filePath'] -Value $content -ErrorAction Stop
                }catch {
